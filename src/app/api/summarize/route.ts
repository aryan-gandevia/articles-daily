...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if summary is already in cache (pre-generated by cron)
    const cached = getCachedDigest();
    if (cached) {
      const cachedArticle = cached.articles.find((a) => a.url === url) as Record<string, unknown> | undefined;
      if (cachedArticle?.summary) {
        return NextResponse.json({
          summary: cachedArticle.summary,
          keyTakeaways: cachedArticle.keyTakeaways || [],
        });
      }
    }
